(function () {
  ...
  const knownKey = "cdl-known-cards";
  const screens = {
  ...
  function byCategory(cat) {
    return window.CDL_QUESTIONS.filter((item) => item.category === cat);
  }
  ...
  function filteredDeck() {
    ...
    let list = window.CDL_FLASHCARDS.slice();
    if (deckFilter === "air-brakes" || deckFilter === "coupling") {
  ...
  function renderBank(filter) {
    ...
    list.innerHTML = "";
    window.CDL_QUESTIONS.filter((item) => filter === "all" || !filter || item.category === filter).forEach((item, i) => {
  ...
  window.startCdlQuiz = function (id) {
    if (id && window.QUIZ_MODES[id]) buildQuiz(id);
  };
  window.startCdlQuiz(new URLSearchParams(window.location.search).get("quiz"));
})();
