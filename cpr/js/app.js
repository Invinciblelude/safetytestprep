(function () {
  ...
  ["CPR_QUESTIONS_SET2", "CPR_QUESTIONS_SET3", "CPR_QUESTIONS_SET4"].forEach((key) => {
    if (window[key] && window[key].length) {
      window.CPR_QUESTIONS = window.CPR_QUESTIONS.concat(window[key]);
    }
  });
  const knownKey = "cpr-known-cards";
  const screens = {
  ...
  let modeId = "heartsaver";
  let queue = [];
  let index = 0;
  let score = 0;
  let missed = [];
  let answered = false;
  let lastMode = "heartsaver";
  ...
  function finishQuiz() {
    const spec = window.QUIZ_MODES[modeId] || { title: "Review", pass: Math.ceil(queue.length * 0.84) };
  ...
  function filteredDeck() {
    ...
    let list = window.CPR_FLASHCARDS.slice();
    if (["numbers", "sequence", "aed", "peds", "firstaid"].indexOf(deckFilter) !== -1) {
  ...
  function renderBank(filter) {
    ...
    list.innerHTML = "";
    window.CPR_QUESTIONS.filter((item) => filter === "all" || !filter || item.category === filter).forEach((item, i) => {
